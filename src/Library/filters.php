...
use function PressGang\get_post;
use function PressGang\get_the_ID;
use function PressGang\is_admin;
use function PressGang\wp_get_post_parent_id;
...
class Filters {
	...
	public static function add() {
		add_filter( 'nav_menu_css_class', array( 'PressGang\Libarary\Filters', 'add_custom_link_menu_item_classes' ), 10, 2 );
		add_filter( 'upload_mimes', array( 'PressGang\Libarary\Filters', 'mime_types' ) );
		add_filter( 'gallery_style', array( 'PressGang\Libarary\Filters', 'gallery_style' ) );
		if ( in_array( 'single-page.php', Config::get( 'templates' ) ) ) {
			add_filter( '_get_page_link', array( 'PressGang\Libarary\Filters', 'single_page_permalink' ), 10, 2 );
			add_filter( 'the_permalink', array( 'PressGang\Libarary\Filters', 'single_page_permalink' ), 10, 2 );
			add_filter( 'get_sample_permalink', array( 'PressGang\Libarary\Filters', 'single_page_permalink' ), 10, 2 );
