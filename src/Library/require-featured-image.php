...
use function PressGang\delete_transient;
use function PressGang\get_transient;
use function PressGang\has_post_thumbnail;
use function PressGang\remove_action;
use function PressGang\set_transient;
use function PressGang\wp_update_post;
...
class RequiredFeaturedImage {
	...
	public function featured_image_error() {
		...
		if ( get_transient( 'has_post_thumbnail' ) == "no" ) { ?>
			<div class="error">
			<p><?php __( "You must select a Featured Image. Your Post has been saved but it cannot be published.", THEMENAME ) ?></p>
			</div><?php
		}
		delete_transient( 'has_post_thumbnail' );
	}
