...
class Block {
	 ...
	 *
	 * @param array<string, mixed> $block The block array. This array contains all the
	 ...
	 * @param $wp_block
	 * @param array<string, mixed>|false $context The context provided to the block by the post or its parent block.
	 */
	public static function render( array $block, string $content, bool $is_preview, int $post_id, mixed $wp_block, array|false $context ): void {
		...
		$slash_position = strpos( $name, '/' );
		$slug           = $slash_position === false ? $name : substr( $name, $slash_position + 1 );
	 ...
	 *
	 * @param array{name: non-empty-string} $block_settings The settings from the block.json file.
	 */
	 ...
	 *
	 * @param array{name: non-empty-string} $block_settings
	 *
