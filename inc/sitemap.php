...
class Sitemap {
	...
	private $frequencies = [ 'never', 'yearly', 'monthly', 'weekly', 'daily', 'hourly', 'always' ];
	...
	public function create_sitemap( $post_id, $post, $update ) {
		...
		$posts = \get_posts( [
			'numberposts' => - 1,
			'orderby'     => [ 'type' => 'ASC', 'menu_order' => 'ASC', 'modified' => 'DESC' ],
		...
		foreach ( $posts as &$post ) {
			...
			if ( function_exists( 'wpml_get_active_languages_filter' ) ) {
				$langs = wpml_get_active_languages_filter( null, [ 'skip_missing' => true ] );
