...
class Filters {
	...
	public static function add() {
		add_filter( 'nav_menu_css_class', [
			'PressGang\Libarary\Filters',
			'add_custom_link_menu_item_classes'
		], 10, 2 );
		add_filter( 'upload_mimes', [ 'PressGang\Libarary\Filters', 'mime_types' ] );
		add_filter( 'gallery_style', [ 'PressGang\Libarary\Filters', 'gallery_style' ] );

		if ( in_array( 'single-page.php', Config::get( 'templates' ) ) ) {
			add_filter( '_get_page_link', [ 'PressGang\Libarary\Filters', 'single_page_permalink' ], 10, 2 );
			add_filter( 'the_permalink', [ 'PressGang\Libarary\Filters', 'single_page_permalink' ], 10, 2 );
			add_filter( 'get_sample_permalink', [ 'PressGang\Libarary\Filters', 'single_page_permalink' ], 10, 2 );
			...
			if ( $parent = get_post( $parent_id ) ) {
				if ( preg_match( '/single-page.php$/', get_page_template_slug( $parent_id ) ) ) {
					...
					if ( is_array( $permalink ) ) {
						...
						if ( ! is_admin() ) {
							$the_id = get_the_ID();
							if ( in_array( $parent_id, [ $the_id, wp_get_post_parent_id( $the_id ) ] ) ) {
